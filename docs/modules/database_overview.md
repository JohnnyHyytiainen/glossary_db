...
Det här är mina breda ämnesområden. De finns till för att du i framtiden ska kunna filtrera fram "Visa mig allt jag kan om SQL".
...
Här sparar jag exakt *varifrån* du fick informationen. Det gör att du slipper skriva samma källa 50 gånger i mina anteckningar, och du kan enkelt uppdatera länkar om en kurs byter plattform.
...
* **Vad som ska in här:** Det specifika ordet, min förklaring av det, och hur svårt det är.
...
* `definition`: Din grymma förklaring av glosan.
* `difficulty`: Nybörjare, mellan eller avancerad (via din `DifficultyLevel` Enum).
...
När du skriver in en `Term` i din Python-kod (i `seed.py`), plockar du helt enkelt upp de `Category`- och `Source`-objekt du redan skapat och "fäster" dem på termen, så här:
